...
/// </para>
/// </summary>
public sealed partial class FreeCamera : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        GameWorld.OnPersonAdd += OnPlayerSpawned;
    }

    private void Start()
    {
        if (DT_Plugin.AZERTYMode.Value)
        {
        ...
        _showOverlay = DT_Plugin.ShowOverlay.Value;
        DT_Plugin.ShowOverlay.SettingChanged += KeybindOverlay_SettingChanged;

        _nightVision = CameraManager.Instance.NightVision;
        _thermalVision = CameraManager.Instance.ThermalVision;

        _freeCameraController = GameWorld.gameObject.GetComponent<FreeCameraController>();
        _originalFov = CameraManager.Instance.Fov;

        var asset = InternalBundleLoader.Instance.GetFreecamUI();
        if (asset == null)
        {
            throw new NullReferenceException("FreecamUI asset is null");
        }
    ...
    {
        if (_currentPlayer != null && _playersTracker.TryGetValue(_currentPlayer.Id, out var listPlayer))
        {
        ...
        if (_currentPlayer != null && _playersTracker.TryGetValue(_currentPlayer.Id, out listPlayer))
        {
            listPlayer.ToggleBackground(true);
        }
#if DEBUG
        DT_Plugin.DT_Logger.LogInfo($"Freecam: Setting player to {_currentPlayer.Profile.GetCorrectedNickname()}");
#endif
    }
    ...
    public List<Player> RecalculateAndGetPlayers()
    {
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Home))
        {
            ...
            NotificationManager.DisplayMessageNotification(_disableInput ? "Freecam input disabled" : "Freecam input enabled");
        }
        ...
        if (_isFollowing)
        {
            if (_currentPlayer != null)
            {
                ...
                {
#if DEBUG
                    DT_Plugin.DT_Logger.LogInfo("Setting left shoulder mode");
#endif
                ...
                {
#if DEBUG
                    DT_Plugin.DT_Logger.LogInfo("Unsetting left shoulder mode");
#endif
            ...
            {
#if DEBUG
                DT_Plugin.DT_Logger.LogInfo("Freecam: currentPlayer vanished while we were following, finding next player to attach to");
#endif
    ...
    public void AttachToPlayer()
    {
        ...
        CheckAndResetFov();
#if DEBUG
        DT_Plugin.DT_Logger.LogInfo($"Freecam: Attaching to helmet cam current player {_currentPlayer.Profile.GetCorrectedNickname()}");
#endif
    ...
    public void AttachToMap()
    {
        ...
        {
#if DEBUG
            DT_Plugin.DT_Logger.LogInfo($"Freecam: Attaching to last tracked player position {_lastKnownPlayerPosition}");
#endif
    ...
    public void Attach3rdPerson()
    {
        ...
        CheckAndResetFov();
#if DEBUG
        DT_Plugin.DT_Logger.LogInfo($"Freecam: Attaching to 3rd person current player {_currentPlayer.Profile.GetCorrectedNickname()}");
#endif
        transform.SetParent(_currentPlayer.PlayerBones.LootRaycastOrigin);
    ...
    private void OnDestroy()
    {
        ...
        if (gameWorld == null)
        {
            throw new NullReferenceException("GameWorld was null during cleanup");
        }
        gameWorld.OnPersonAdd -= OnPlayerSpawned;
        foreach (var player in GameWorld.AllAlivePlayersList)
        {
